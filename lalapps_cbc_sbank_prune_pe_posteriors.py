...
#    parser.add_option("--ns-spin-max", type=float, metavar="SPIN", help="Maximum spin for neutron stars when using --ns-bh-boundary-mass.")
    ...
    parser.add_option("--pe-samples", metavar="FILE", help="PE posteriors.")
...
# initialize the bank
#
bank = Bank(noise_model, opts.flow, False, opts.cache_waveforms, opts.neighborhood_size, opts.neighborhood_param, coarse_match_df=opts.coarse_match_df, iterative_match_df_max=opts.iterative_match_df_max, fhigh_max=opts.fhigh_max)
...
proposal = (InternalTemplate.from_sngl(t, bank=bank)
...
reject = 0
df = 0.5
PSD = get_PSD(df, bank.flow, opts.fhigh_max, noise_model)
...
for tmplt in proposal:
    ...
    # NOTE: Need to use the cross term when normalizing
    ...
    hp.data.data = tmp_H1
    hc.data.data = tmp_L1
    #tmp_h1 = fp_H1 * data_hp + fc_H1 * data_hc
    #tmp_l1 = fp_L1 * data_hp + fc_L1 * data_hc
    tmplt.h_H1 = hp
    tmplt.h_L1 = hc
    
    max_match = 0
    for point in bank:
        match_H1 = InspiralSBankComputeMatch(tmplt.h_H1, point.h_H1, bank._workspace_cache[0])
        match_L1 = InspiralSBankComputeMatch(tmplt.h_L1, point.h_L1, bank._workspace_cache[0])
        #print match_H1, match_L1
        if match_H1 > 0.99 and match_L1 > 0.99:
            reject += 1
            break
        if match_H1 > max_match:
            max_match = match_H1
        if match_L1 > max_match:
            max_match = match_L1
        if max_match > 1.008:
            print "RAISING", max_match, match_H1, match_L1
            raise ValueError()
    else:
        bank.insort(tmplt)
    print len(bank), reject, max_match
